...
namespace mpmt
{
    ...
    public:
        ...
        virtual uint64_t compute(const T* const data, const size_t len) = 0;
        virtual uint64_t compute(const std::vector<T>& data) = 0;

    private:
        static inline uint64_t m_polynomial = 0x42F0E1EBA9EA3693;
        static inline uint64_t m_table[256] = {
